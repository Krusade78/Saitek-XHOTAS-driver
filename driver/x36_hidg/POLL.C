...
{
    ULONG               BeforeTimes[16];//MAX_AXES*MAX_AXES];
    ULONG               AfterTimes[16];//MAX_AXES*MAX_AXES];
...
NTSTATUS HGM_UpdateLatestPollData
    ( 
    ...
    )
{
    ...
    KIRQL               oldIrql;
    LONG                axisIdx;
	ULONG				ejes[4];//MAX_AXES];

	RtlZeroMemory(ejes,sizeof(ULONG)*4);//MAX_AXES);
	...
	for(axisIdx=0;axisIdx<4/*MAX_AXES*/;axisIdx++) {
		if( (ejes[axisIdx]!=DeviceExtension->PrevGoodAxis[axisIdx]) && (DeviceExtension->RepsAxis[axisIdx]<1) ) {
			DeviceExtension->RepsAxis[axisIdx]++;
			DeviceExtension->LastGoodAxis[axisIdx]=DeviceExtension->PrevGoodAxis[axisIdx];
		} else {
			DeviceExtension->RepsAxis[axisIdx]=0;
			DeviceExtension->LastGoodAxis[axisIdx]=ejes[axisIdx];
			DeviceExtension->PrevGoodAxis[axisIdx]=DeviceExtension->LastGoodAxis[axisIdx];
		}
	}
