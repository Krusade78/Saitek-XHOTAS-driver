...
{ 
    ...
    { 
		MessageBox(NULL,"StartServiceCtrlDispatcher error","[X45-Service]",MB_SERVICE_NOTIFICATION|MB_ICONSTOP);
    } 
} 
...
void InicioServicio (DWORD argc, LPTSTR *argv)
{
	...
	svHandle = RegisterServiceCtrlHandlerEx( 
        "X45-Service", ServicioCtrlHandler,NULL); 
 
    if (svHandle == (SERVICE_STATUS_HANDLE)0) 
    { 
		MessageBox(NULL,"RegisterServiceCtrlHandler error","[X45-Service]",MB_SERVICE_NOTIFICATION|MB_ICONSTOP);
        return; 
    }
...
/*    if (!serv->IniciarServicio()) 
    ...
    { 
		MessageBox(NULL,"SetServiceStatus error","[X45-Service]",MB_SERVICE_NOTIFICATION|MB_ICONSTOP);
		delete serv; serv=NULL;
		return;
    }
}
...
)
{
    ...
    if (!SetServiceStatus (svHandle,  &svEstado)) 
    {
		MessageBox(NULL,"Handler SetServiceStatus error","[X45-Service]",MB_SERVICE_NOTIFICATION|MB_ICONWARNING);
		return 6;
	} else {
		if(salir && serv!=NULL) {
			delete serv; serv=NULL;
